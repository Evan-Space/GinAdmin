				...
				Children: []RouteGroupDef{
					{
						...
						Routes: []RouteDef{
							...
							POST("adminList", "管理员列表", AuthPerm, adminUserCtrl.List),
							GET("memberListName", "普通成员名字枚举值", AuthPerm, adminUserCtrl.MemberListName),
							POST("memberList", "普通成员列表", AuthPerm, adminUserCtrl.MemberList),
							...
							POST("changeAccountStatus", "修改账号状态", AuthPerm, adminUserCtrl.ChangeAccountStatus),
						},
					},
					{
						Prefix: "role",
